...
- 달, 갈릴레이 위성, 타이탄, 엔셀라두스, 트리톤 등 대표 위성 선택
- 행성별 자전축 기울기와 선택 시 축 가이드 표시
- 교육용으로 압축한 거리/크기 표현
- 재생, 일시정지, 리셋, 시간 속도 조절
- 천체 선택 시 카메라 포커스 이동
- 실제 거리, 지름, 공전 주기, 자전 주기, 위성 부모 행성 정보 패널
- NASA/JPL/LRO 계열 공개 텍스처 기반 행성/위성 표면
- 데스크톱과 모바일 반응형 UI
- GitHub Pages 자동 배포
...
행성과 위성 표면은 NASA 3D Resources, NASA SVS CGI Moon Kit, JPL Solar System Simulator texture maps의 공개 자료를 로컬 WebP로 변환해 사용합니다.
